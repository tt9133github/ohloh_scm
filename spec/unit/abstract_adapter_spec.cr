...
describe "AbstractAdapter" do
  ...
  it "invalid_urls" do
    [nil, "", "*" * 121].each do |url|
      AbstractAdapter.new({:url => url}).validate_url.any?.should be_truthy
    end
  end

  it "invalid_usernames" do
    ["no spaces allowed", "/", ":", "a"*33].each do |username|
      AbstractAdapter.new({:username => username}).validate_username.any?.should be_truthy
    end
  end
  ...
  it "invalid_passwords" do
    ["no spaces allowed", "a"*33].each do |password|
      AbstractAdapter.new({:password => password}).validate_password.any?.should be_truthy
    end
  end
  ...
  it "invalid_branch_names" do
    ["%","a"*81].each do |branch_name|
      AbstractAdapter.new({:branch_name => branch_name}).validate_branch_name.any?.should be_truthy
    end
  end
