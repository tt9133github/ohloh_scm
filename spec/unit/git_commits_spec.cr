...
describe "GitCommits" do
  ...
  it "commits" do
    with_git_repository("git") do |git|
      ...
      git.commits(after: "1df547800dcd168e589bb9b26b4039bff3a7f7e4").should eq(Array(Commit).new)
    end
  end
  ...
  it "each_commit_with_null_merge" do
    with_git_repository("git_with_null_merge") do |git|
      git.each_commit do |c|
        c.diffs.empty?.should be_true if c.token == "d3bd0bedbf4b197b2c4eb827e1ec4c35b834482f"
      end
    end
  end
  ...
  it "log_encoding" do
    with_git_repository("git_with_invalid_encoding") do |git|
      git.log(nil, nil, false).valid_encoding?.should eq(true)
    end
  end

  it "verbose_commits_valid_encoding" do
    with_git_repository("git_with_invalid_encoding") do |git|
      git.verbose_commit("8d03f4ea64fcd10966fb3773a212b141ada619e1").message.to_s.valid_encoding?.should be_true
    end
  end

  it "open_log_file_encoding" do
    with_git_repository("git_with_invalid_encoding") do |git|
      git.open_log_file(0, nil, false) do |io|
