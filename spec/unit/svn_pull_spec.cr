...
describe "SvnPull" do
  ...
  it "svnadmin_create_local" do
    OhlohScm::ScratchDir.new do |dir|
      ...
      svn.exist?.should be_truthy
      File.exists?(File.join(dir, "hooks", "pre-revprop-change")).should be_truthy
      File.executable?(File.join(dir, "hooks", "pre-revprop-change")).should be_truthy
  ...
  it "svnadmin_create_remote" do
    OhlohScm::ScratchDir.new do |dir|
      svn = SvnAdapter.new(url: "svn+ssh://#{System.hostname}#{dir}")
