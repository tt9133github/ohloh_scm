...
describe "HgParser" do
  ...
  it "log_parser_verbose" do
    ...
    commits[0].committer_name.should eq("Jason Allen")
    commits[0].committer_email.should eq("jason@ohloh.net")
    commits[0].message.should eq("added makefile") # NOTE: no \n at end of comment
    commits[0].committer_date.should eq(Time.utc(2009,1,20,19,33,17))
    ...
    commits[1].committer_name.should eq("Robin Luckey")
    commits[1].committer_email.should eq("robin@ohloh.net")
    commits[1].message.should eq("Initial Checkin") # NOTE: no \n at end of comment
    commits[1].committer_date.should eq(Time.utc(2009,1,20,19,32,54))
  ...
  it "styled_parser" do
    with_hg_repository("hg") do |hg|
      File.exists?(HgStyledParser.style_path).should be_truthy
      ...
      File.exists?(HgStyledParser.verbose_style_path).should be_truthy
...
def assert_styled_commits(commits, with_diffs=false)
  ...
  commits[1].committer_name.should eq("Robin Luckey")
  commits[1].committer_email.should eq("robin@ohloh.net")
  (Time.utc(2009,1,20,19,34,53) - commits[1].committer_date.as(Time)).to_i.should be < 1 # Don"t care about milliseconds
  ...
  (Time.utc(2009, 1,20,19,34,4) - commits[2].committer_date.as(Time)).to_i.should be < 1
