...
describe "SvnParser" do
  ...
  it "empty_xml" do
    # FIXME: XmlWriter is not used.
    # SvnParser.parse("", writer: XmlWriter.new ).should eq("<?xml version=\"1.0\"?>\n<ohloh_log scm=\"svn\">\n</ohloh_log>\n")
  end

  it "parse must work correctly" do
    commits = Array(String).new
    SvnParser.parse(File.read("#{DATA_DIR}/simple.svn_log")) do |commit|
      commits << commit.token.to_s
    end
    commits.should eq(%w[5 4 3 2 1])
  end

  it "log_parser" do
    sample_log = <<-SAMPLE
    ------------------------------------------------------------------------
    r1 | robin | 2006-06-11 11:28:10 -0700 (Sun, 11 Jun 2006) | 2 lines
    ...
    revs[0].committer_date.should eq(Time.utc(2006,6,11,18,28,10))
