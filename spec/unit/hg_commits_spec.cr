...
describe "HgCommits" do
  ...
  it "commit_count_with_empty_branch" do
    with_hg_repository("hg", nil) do |hg|
  ...
  it "each_commit" do
    ...
    with_hg_repository("hg") do |hg|
      hg.each_commit do |c|
        c.token.to_s.size.should eq 40
        c.committer_name.should be_truthy
        c.committer_date.should_not be_nil
      ...
      File.exists?(hg.log_filename).should be_falsey # Make sure we cleaned up after ourselves
