...
describe "GitStyledParser" do
  it "basic" do
    ...
    helloworld = File.read(File.dirname(__FILE__) + "/../data/helloworld.log")
    ...
    commits.each do |commit|
      # puts commit.inspect
      commit.token.to_s.size.should eq(40)
      ...
      commit.diffs.each do |d|
        d.sha1.to_s.size.should eq(40)
        d.parent_sha1.to_s.size.should eq(40)
        d.path.to_s.empty?.should be_false
        d.action =~ /[ACDMRTUXB]/.should be_truthy
      end
    end

    commits[0].author_date.should eq(Time.utc(2006,6,11,11,28,0))
    commits[1].author_date.should eq(Time.utc(2006,6,11,18,32,13))
    commits[2].author_date.should eq(Time.utc(2006,6,11, 9,34,17))
