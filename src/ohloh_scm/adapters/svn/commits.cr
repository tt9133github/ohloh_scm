...
module OhlohScm::Adapters
  class SvnAdapter < AbstractAdapter
    ...
    property final_token : StringOrNil

    # Returns the count of commits following revision number 'after'.
    def commit_count(after = 0)
      after = after.to_i
      return 0 if final_token && after >= final_token.as(Int32)
      run(%(svn log --trust-server-cert --non-interactive -q -r #{after + 1}:#{final_token || "HEAD"} --stop-on-copy '#{SvnAdapter.uri_encode(File.join(root, branch_name.to_s))}@#{final_token || "HEAD"}' | grep -E -e '^r[0-9]+ ' | wc -l)).strip.to_i
    end

    # Returns an array of revision numbers for all commits following revision number 'after'.
    def commit_tokens(after = 0)
      after = after.to_i
      return Array(Int32).new if final_token && after >= final_token.as(Int32)
    ...
    def commits(after = 0)
      ...
      open_log_file(after.try &.to_i) do |io|
        list = OhlohScm::Parsers::SvnXmlParser.parse(io)
      end
      # TODO: Find Why is compile-time type String | Commit?
      list.map do |c|
        commit = c.as(Commit)
        commit.scm = self
        commit
    ...
    #
    def each_commit(after = 0)
      commit_tokens(after).each do |rev|
    ...
    def deepen_commit(commit)
      deep_commit = commit.dup
      if commit.diffs
        deep_commit.diffs = commit.diffs.map do |diff|
          deepen_diff(diff, commit.token)
        end.compact.flatten.uniq { |diff| diff.action.to_s && diff.path.to_s }.sort_by { |diff| diff.action.to_s && diff.path.to_s }
      end
    ...
    def deepen_diff(diff, rev)
      ...
      int_rev = rev.try(&.to_i) || 0
      recurse_rev = (diff.action == "D") ? int_rev - 1 : rev
      ...
      if (diff.action == "D" || diff.action == "A") && is_directory?(diff.path, recurse_rev)
        ...
        recurse_files(diff.path, recurse_rev).map do |f|
          OhlohScm::Diff.new(action: diff.action, path: File.join(diff.path.to_s, f))
        end
      else
    ...
    def strip_diff_branch(diff)
      stripped_diff = diff.dup
      stripped_diff.path = strip_path_branch(diff.path)
      stripped_diff if stripped_diff.path
    end
    ...
    def strip_path_branch(path)
      if path == self.branch_name.to_s
        ...
        $1 if path =~ /^#{Regex.escape(self.branch_name.to_s)}(\/.*)$/
      end
    end
    ...
    def open_log_file(after)
      after ||= 0
      begin
        final_token_int = final_token.try(&.to_i) || 0
        if final_token && after >= final_token_int || head_token && after >= head_token.as(Int32)
          ...
          run %(svn log --trust-server-cert --non-interactive --xml --stop-on-copy -r #{after + 1}:#{final_token || "HEAD"} '#{SvnAdapter.uri_encode(File.join(self.root, self.branch_name.to_s))}@#{final_token || "HEAD"}' #{opt_auth} | #{ string_encoder } > #{log_filename})
        end
    ...
    def single_revision_xml(revision)
      run "svn log --trust-server-cert --non-interactive --verbose --xml --stop-on-copy -r #{revision} --limit 1 #{opt_auth} '#{SvnAdapter.uri_encode(File.join(self.root, self.branch_name.to_s))}@#{revision}' | #{ string_encoder }"
    end
    ...
    def recurse_files(path=nil, revision=final_token || "HEAD")
      ...
      return files unless stdout
      stdout.each_line do |s|
        s = s.chomp
        # TODO: check if we have issues without this: s.force_encoding("UTF-8")
        files << s if s.size > 0 && s !~ /CVSROOT\// && s[-1..-1] != "/"
      end
      files.sort
    end
  end
