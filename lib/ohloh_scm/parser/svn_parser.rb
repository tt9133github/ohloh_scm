...
module OhlohScm
  class SvnParser < Parser
    ...
    def self.internal_parse(buffer, _opts)
      e = nil
      state = :data
      previous_state = nil
      previous_line = nil
      ...
      buffer.each_line do |l|
        ...
        if state == :data
          ...
          if l =~ /^   (\w) ([^\(\)]+)( \(from (.+):(\d+)\))?$/
            e.diffs ||= []
            e.diffs << OhlohScm::Diff.new(action: Regexp.last_match(1), path: Regexp.last_match(2), from_path: Regexp.last_match(4), from_revision: Regexp.last_match(5).to_i)
          else
            next_state = :comment
          end
